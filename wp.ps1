...
if (!$dest) { Write-Host "destenation directory not set"; exit(1)}
...
if (![System.IO.Directory]::Exists($dest)) {
    write-host "Destenation directory not found"
    if ($y) {
...
foreach ($currentImage in $srcFiles) {
    ...
    if ($w -gt $h) {
        write-host "$currentImage is landscape $w"
        if ($l) {
            if ($minwidth -and $w -lt $minwidth) {
                Write-Host "skipping"
                continue;
            }
            ...
            try {
                ...
                Copy-Item $currentImage.FullName $dest\$NewName
            }
            catch {
    ...
    if ($h -gt $w) {
        Write-Host "$currentImage is portrait $h"
        if ($p) {
            write-host "copying to $dest"
            try {
